...
// export default function EditPublicationPage({ editingPub, onUpdatePublication, setCurrentPage }) {
...
//       setReleaseDate(editingPub.releaseDate)
//     }
//   }, [editingPub])
...
//     } else {
//       coverUrl = editingPub.coverUrl // fallback ke cover lama
//     }
...
//           />
//           <p className="text-sm text-gray-500 mt-1">Biarkan kosong kalau mau pakai cover yang lama</p>
//         </div>
...
import { uploadImageToCloudinary } from '../services/publicationServices';
...
export default function EditPublicationPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await editPublication(updatedPublication); // penting: tambahin await
      navigate('/publications');
    } catch (err) {
      alert('Gagal update publikasi: ' + err.message);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          ...
          <input
            ...
            onChange={e => setTitle(e.target.value)}
            className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-sky-500 focus:border-sky-500"
            placeholder="Contoh: Indikator Ekonomi DK Jakarta 2025"
          />
        </div>
        <div>
          <label htmlFor="releaseDate" className="block text-sm font-medium text-gray-700 mb-1">Tanggal Rilis</label>
